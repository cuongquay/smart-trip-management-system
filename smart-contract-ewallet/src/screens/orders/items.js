...
const CircleItem = props => {
  ...
  const { size, status, activity, iconStyle } = props;
  ...
  return (
    <View
      style={{
        ...
        borderRadius: Size / 2,
        borderColor:
          status && status !== 'completed' ? Colors.RED : Colors.BLACK,
    ...
    >
      <Icon
        type="ionicon"
        style={[activity ? activity.style : null, iconStyle]}
        color={Colors.WHITE}
        name={activity ? activity.icon : null}
      />
    </View>
  );
};
...
export { OrderItem, CircleItem, getStatus };
