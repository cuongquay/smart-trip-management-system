...
function printHelp () {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"tripcontractchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c tripcontractchannel"
  echo "	byfn.sh up -c tripcontractchannel -s couchdb"
  echo "        byfn.sh up -c tripcontractchannel -s couchdb -i 1.1.0-alpha"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c tripcontractchannel"
  echo "        byfn.sh upgrade -c tripcontractchannel"
  echo
...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.tripcontract.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.tripcontract.com
  docker cp -a orderer.tripcontract.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.tripcontract.com
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.tripcontract.com

  for PEER in peer0.trade.tripcontract.com peer1.trade.tripcontract.com peer0.trade.booking.com peer1.trade.booking.com; do
